 ...
 */
import {AbortablePromise, arrays, CancelError, Deferred, PromiseCreator, promises} from '../../src/index';
...
describe('promises', () => {
  ...
  describe('AbortablePromise', () => {
    ...
    describe('abort', () => {
      ...
      it('rejects the promise with CancelError', async () => {
        const abortablePromise = new AbortablePromise<void>(() => {
        });
        abortablePromise.abort();
        await expectAsync(abortablePromise).toBeRejectedWith(new CancelError());
      });
